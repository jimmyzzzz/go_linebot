...
class BASE_ROOT:
	...
	def run(self):
		...
		if run_start_ret: return_str+=run_start_ret
		...
		while self.run_index < cmds_num:
			...
			try:
				...
				error_message += '='*20+'\n'
				return_str += error_message
				
				run_error_ret = self.run_error()
				if run_error_ret: return_str+=run_error_ret
				return return_str
			
			self.run_index+=1
		
		run_end_ret = self.run_end()
		if run_end_ret: return_str+=run_end_ret
		return return_str
