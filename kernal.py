...
class PercentageKernal(DirKernal):
	...
	def import_root(self, root_name):
	...
	def deconstruct_CMD(self, CMD):
		...
		if ":" not in first_cmd:
			...
			return user_id, build_cmd, root_cmd, user_cmds

		(build_cmd, root_cmd) = first_cmd.split(':', 1)
		return user_id, build_cmd, root_cmd, user_cmds

	def get_RootOpt_CmdData(self, CMD):
		(user_id, build_cmd, root_cmd, user_cmds) = self.deconstruct_CMD(CMD)
		...
		if root_name in self.root_dict:
			...
			root = self.import_root(root_name)
			self.root_dict[root_name] = root

		RootOpt = (root_name, root, options)
		CmdData = (user_id, root_cmd, user_cmds)
